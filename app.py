...
st.set_page_config(page_title="Carbon Offset Engine", page_icon="🌱", layout="wide")
...
BUILD_CARBON_KG_PER_MW = {
    "Solar": 670000, "Wind": 80616, "Hydro": 2500000, "Nuclear": 1025000, "Biomass": 500000,
}

RENEWABLE_COLOURS = {
    "Solar": "#f39c12", "Wind": "#2ecc71", "Hydro": "#3498db",
    "Nuclear": "#9b59b6", "Biomass": "#e74c3c",
}
...
def predict_species_recovery(plant_row, opening_year, closure_year):
    ...
    if invalid_dist:
        return {
            ...
            "reason":               "Habitat distance data is unreliable for this location. "
                                    "The site may be on an island, coastal area, or outside "
                                    "the CEH Land Cover Map coverage area.",
        }
    ...
    # ── years 15 and 20 — single ML call at years_since=15 ───
    # model plateaus after 15 years so one call gives both anchors
    ...
    # use ML if above baseline — don't force above simple recovery
    # ML is often more conservative than simple 2.5%/yr which is honest
    sp_15yr = max(species_before, ml_15yr) if ml_15yr > species_before * 0.8 else simple_recovery(15)
    sp_20yr = max(species_before, ml_20yr) if ml_20yr > species_before * 0.8 else simple_recovery(20)
...
def get_roi_for_renewable(plant_row, renewable_type):
    ...
    return {
        "co2_value": co2_value, "water_value": water_value,
        "nox_value": nox_value, "sox_value": sox_value, "ch4_value": ch4_value,
...
if st.sidebar.button("Calculate ROI", type="primary"):
    ...
    co2_labels = ["Total CO2\n(current)", "Solar\nsaving", "Wind\nsaving",
                  "Hydro\nsaving", "Nuclear\nsaving", "Biomass\nsaving"]
    co2_values = [
        struct_row["CO2_Total CO2 per year"], struct_row["CO2_Solar saving"],
        struct_row["CO2_Wind saving"], struct_row["CO2_Hydro saving"],
        struct_row["CO2_Nuclear saving"], struct_row["CO2_Biomass saving"],
    ]
    fig_co2, ax_co2 = plt.subplots(figsize=(10, 5))
    ax_co2.bar(co2_labels, co2_values, color=["#e74c3c"]+["#2ecc71"]*5, edgecolor="white")
    ...
    emissions_config = {
        "NOX": {"total": "Emissions_NOX kg", "savings": [f"Emissions_{r} NOX saving" for r in RENEWABLES]},
        "SOX": {"total": "Emissions_SOX kg", "savings": [f"Emissions_{r} SOX saving" for r in RENEWABLES]},
        "CH4": {"total": "Emissions_CH4 kg", "savings": [f"Emissions_{r} CH4 saving" for r in RENEWABLES]},
    }
    for ax_em, (em_type, cfg) in zip(axes_em, emissions_config.items()):
        ...
        ax_em.bar(["Current"]+RENEWABLES, em_values, color=["#e74c3c"]+["#3498db"]*5, edgecolor="white")
    ...
    land_values = [
        struct_row["Land_Site ha"], struct_row["Land_Solar net land"],
        struct_row["Land_Wind net land"], struct_row["Land_Hydro net land"],
        struct_row["Land_Nuclear net land"], struct_row["Land_Biomass net land"],
    ]
    land_colors = ["#e74c3c"] + ["#2ecc71" if v >= 0 else "#f39c12" for v in land_values[1:]]
    fig_land, ax_land = plt.subplots(figsize=(10, 5))
    ax_land.bar(["Site\n(current)", "Solar\nnet land", "Wind\nnet land",
                 "Hydro\nnet land", "Nuclear\nnet land", "Biomass\nnet land"],
                land_values, color=land_colors, edgecolor="white")
    ...
    for renewable in RENEWABLES:
        ax_payback.plot(years, co2_timelines[renewable],
    ...
    payback_data = {}
    for renewable in RENEWABLES:
        payback = next((y for y, v in zip(years, co2_timelines[renewable]) if v > 0), None)
        payback_data[renewable] = f"Year {payback}" if payback else "Not within 20 years"
    st.dataframe(pd.DataFrame(payback_data, index=["Carbon Payback Year"]), use_container_width=True)
    st.markdown("---")
    ...
    table_data = {
        "": ["CO2 savings", "Water savings", "NOX savings", "SOX savings",
             "CH4 savings", "Land value", "Species recovery", "Renewable bio impact", "NET ANNUAL"],
        "Close Only": [
            f"£{baseline_roi['co2_value']/1e6:.2f}M", f"£{baseline_roi['water_value']/1e6:.2f}M",
            f"£{baseline_roi['nox_value']/1e6:.2f}M", f"£{baseline_roi['sox_value']/1e6:.2f}M",
            f"£{baseline_roi['ch4_value']/1e6:.2f}M", f"£{land_value_baseline/1e6:.2f}M",
            f"£{species_annual/1e6:.2f}M", "£0.00M", f"£{baseline_annual/1e6:.2f}M",
        ],
    }
    for r in RENEWABLES:
        table_data[r] = [
            f"£{results[r]['co2_value']/1e6:.2f}M", f"£{results[r]['water_value']/1e6:.2f}M",
            f"£{results[r]['nox_value']/1e6:.2f}M", f"£{results[r]['sox_value']/1e6:.2f}M",
            f"£{results[r]['ch4_value']/1e6:.2f}M", f"£{results[r]['land_value']/1e6:.2f}M",
            f"£{results[r]['species_value']/1e6:.2f}M", f"£{results[r]['bio_impact']/1e6:.2f}M",
    ...
    ax_net.axhline(baseline_annual/1e6, color="#95a5a6", linewidth=1.5, linestyle="--", label="Close only baseline")
    ...
    for renewable in RENEWABLES:
        ...
        ax_cum.plot(years, cumulative_line, color=RENEWABLE_COLOURS[renewable], linewidth=2, label=renewable)
    ...
    if prediction and not prediction.get("prediction_reliable", True):
        ...
        for i, cal_year in enumerate(calendar_years):
            species_at_year = trajectory[i]
            uplift_at_year  = max(0, (species_at_year - sp_before) / sp_before)
            annual_val      = ECOSYSTEM_VALUE_PER_HA_YR * uplift_at_year * site_ha_v
            running        += annual_val
